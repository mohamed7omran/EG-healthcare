...
export function BookingModal({ doctor, isOpen, onClose }: BookingModalProps) {
  const { addAppointment, currentUserId, user } = useApp();
  const [selectedDate, setSelectedDate] = useState<Date | undefined>(undefined);
  const [selectedTime, setSelectedTime] = useState<string | null>(null);
  const [step, setStep] = useState<'date' | 'time' | 'confirm' | 'success'>('date');

  const mockPatient = mockPatients.find(p => p.id === currentUserId);
  const patientName = mockPatient?.name ?? user?.displayName ?? user?.email ?? 'Patient';
          ...
          <div className="space-y-6 py-4 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {selectedDate && format(selectedDate, 'EEEE, MMM dd, yyyy')} at {selectedTime}
              </p>
            </div>
            <Button onClick={handleClose} className="w-full gradient-primary border-0">
              Done
            </Button>
          </div>
        )}
